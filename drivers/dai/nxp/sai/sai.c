 ...
 *	SYNC and ASYNC for each direction. As such, there's 4 possible combinations
 ...
 *		d) TX SYNC, RX SYNC
 *
 *	Combination d) is not valid and is covered by a BUILD_ASSERT(). As such, there's 3 valid
 *	combintations that need to be supported. Since the main branch of the IF statement covers
 *	combination a), there's only combinatons b) and c) to be covered here.
 *
