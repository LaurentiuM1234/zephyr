...
#define UINT_TO_I2S(x) ((I2S_Type *)(x))
...
#define SAI_TX_RX_SW_RESET(dir, regmap)\
	((dir) == DAI_DIR_RX ? SAI_RxSoftwareReset(UINT_TO_I2S(regmap), kSAI_ResetTypeSoftware) :\
	 SAI_RxSoftwareReset(UINT_TO_I2S(regmap), kSAI_ResetTypeSoftware))
 ...
 * macro when disabling transmitter/reciever.
 */
...
struct sai_config {
	 ...
	 * to be assumed that the watermark should be set to helf of the FIFO
	 * size.
	 */
...
static inline void get_serial_default_config(sai_serial_data_t *cfg)
{
	 ...
	 * 2) MSB is first.
	 */
	/* note: this is ignored if xCR4 doesn't have CHMOD */
#ifdef CONFIG_SAI_HAS_XCR4_CHMOD
	cfg->dataMode = kSAI_DataPinStateOutputZero;
#endif /* CONFIG_SAI_HAS_XCR4_CHMOD */
			    ...
			    enum dai_state new_state)
{
	...
	case DAI_STATE_READY:
		/* TODO: for some odd reason, SOF calls
		 * set_config() twice via the dai-legacy API.
		 * For now, this will be allowed.
		 *
		 * note: we also allow a set_config() operation
		 * after a trigger_stop() operation.
		 */
