...
#ifndef ZEPHYR_DRIVERS_DMA_DMA_NXP_IMX_EDMA_H_
#define ZEPHYR_DRIVERS_DMA_DMA_NXP_IMX_EDMA_H_
...
LOG_MODULE_REGISTER(nxp_imx_edma);

/* used for driver binding */
#define DT_DRV_COMPAT nxp_imx_edma
...
#define EDMA_HAL_CFG_GET(inst)								\
	COND_CODE_1(DT_NODE_HAS_PROP(DT_INST(inst, DT_DRV_COMPAT), hal_cfg_index),	\
		    (edma_hal_configs[DT_INST_PROP(inst, hal_cfg_index)]),		\
		    (edma_hal_configs[0]))
...
struct edma_data {
	...
	struct edma_hal_config *hal_cfg;
};
...
static inline int get_transfer_type(enum dma_channel_direction dir, uint32_t *type)
{
	...
	case MEMORY_TO_MEMORY:
		*type = EDMA_MEMORY_TO_MEMORY;
		break;
	case MEMORY_TO_PERIPHERAL:
		*type = EDMA_MEMORY_TO_PERIPHERAL;
		break;
	case PERIPHERAL_TO_MEMORY:
		*type = EDMA_PERIPHERAL_TO_MEMORY;
		break;
	default:
					       ...
					       uint32_t chan_id)
{
	...
	LOG_DBG("CH_CSR: 0x%x",
		edma_hal_channel_reg_read(data->hal_cfg, chan_id, EDMA_TCD_CH_CSR));
	LOG_DBG("CH_ES: 0x%x",
		edma_hal_channel_reg_read(data->hal_cfg, chan_id, EDMA_TCD_CH_ES));
	LOG_DBG("CH_INT: 0x%x",
		edma_hal_channel_reg_read(data->hal_cfg, chan_id, EDMA_TCD_CH_INT));
	LOG_DBG("CH_SBR: 0x%x",
		edma_hal_channel_reg_read(data->hal_cfg, chan_id, EDMA_TCD_CH_SBR));
	LOG_DBG("CH_PRI: 0x%x",
		edma_hal_channel_reg_read(data->hal_cfg, chan_id, EDMA_TCD_CH_PRI));

	if (EDMA_HAS_MUX(data->hal_cfg)) {
		LOG_DBG("CH_MUX: 0x%x",
			edma_hal_channel_reg_read(data->hal_cfg, chan_id, EDMA_TCD_CH_MUX));
	}

	LOG_DBG("TCD_SADDR: 0x%x",
		edma_hal_channel_reg_read(data->hal_cfg, chan_id, EDMA_TCD_SADDR));
	LOG_DBG("TCD_SOFF: 0x%x",
		edma_hal_channel_reg_read(data->hal_cfg, chan_id, EDMA_TCD_SOFF));
	LOG_DBG("TCD_ATTR: 0x%x",
		edma_hal_channel_reg_read(data->hal_cfg, chan_id, EDMA_TCD_ATTR));
	LOG_DBG("TCD_NBYTES: 0x%x",
		edma_hal_channel_reg_read(data->hal_cfg, chan_id, EDMA_TCD_NBYTES));
	LOG_DBG("TCD_SLAST_SDA: 0x%x",
		edma_hal_channel_reg_read(data->hal_cfg, chan_id, EDMA_TCD_SLAST_SDA));
	LOG_DBG("TCD_DADDR: 0x%x",
		edma_hal_channel_reg_read(data->hal_cfg, chan_id, EDMA_TCD_DADDR));
	LOG_DBG("TCD_DOFF: 0x%x",
		edma_hal_channel_reg_read(data->hal_cfg, chan_id, EDMA_TCD_DOFF));
	LOG_DBG("TCD_CITER: 0x%x",
		edma_hal_channel_reg_read(data->hal_cfg, chan_id, EDMA_TCD_CITER));
	LOG_DBG("TCD_DLAST_SGA: 0x%x",
		edma_hal_channel_reg_read(data->hal_cfg, chan_id, EDMA_TCD_DLAST_SGA));
	LOG_DBG("TCD_CSR: 0x%x",
		edma_hal_channel_reg_read(data->hal_cfg, chan_id, EDMA_TCD_CSR));
	LOG_DBG("TCD_BITER: 0x%x",
		edma_hal_channel_reg_read(data->hal_cfg, chan_id, EDMA_TCD_BITER));
}
				  ...
				  uint32_t chan_id)
{
	...
	if (transfer_type == EDMA_PERIPHERAL_TO_MEMORY) {
		slast = 0;
	} else {
	...
	if (transfer_type == EDMA_MEMORY_TO_PERIPHERAL) {
		dlast = 0;
	} else {
	...
	edma_hal_channel_reg_write(data->hal_cfg, chan_id, EDMA_TCD_SLAST_SDA, slast);
	edma_hal_channel_reg_write(data->hal_cfg, chan_id, EDMA_TCD_DLAST_SGA, dlast);
...
#endif /* ZEPHYR_DRIVERS_DMA_DMA_NXP_IMX_EDMA_H_ */
