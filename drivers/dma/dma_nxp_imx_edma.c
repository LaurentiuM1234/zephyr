...
static void edma_isr(const void *parameter)
{
	...
	if (!edma_hal_channel_reg_read(data->hal_cfg, chan->id, EDMA_TCD_CH_INT)) {
		/* skip, interrupt was probably triggered by another channel */
		return;
	}

	/* clear interrupt */
	edma_hal_channel_reg_update(data->hal_cfg, chan->id,
				    EDMA_TCD_CH_INT, EDMA_TCD_CH_INT_MASK, 0);

	if (chan->cyclic_buffer) {
		update_size = chan->bsize;

		if (IS_ENABLED(CONFIG_DMA_NXP_IMX_EDMA_ENABLE_HALFMAJOR_IRQ)) {
		       ...
		       struct dma_config *dma_cfg)
{
	...
	if (!edma_hal_transfer_width_is_valid(data->hal_cfg,
	...
	ret = edma_hal_configure_transfer(data->hal_cfg, chan_id,
					  dma_cfg->head_block->source_address,
					  dma_cfg->head_block->dest_address,
					  dma_cfg->source_data_size,
					  dma_cfg->dest_data_size,
	...
	if (EDMA_HAS_MUX(data->hal_cfg)) {
		ret = edma_hal_set_channel_mux(data->hal_cfg, chan_id,
					       dma_cfg->dma_slot);
		if (ret < 0) {
	 ...
	 */
	edma_hal_channel_reg_update(data->hal_cfg, chan_id,
				    EDMA_TCD_CSR, EDMA_TCD_CSR_INTMAJOR_MASK,
				    0);
			   ...
			   struct dma_status *stat)
{
	...
	if (chan->cyclic_buffer) {
		 ...
		 */
		citer = edma_hal_channel_reg_read(data->hal_cfg,
						  chan_id,
						  EDMA_TCD_CITER);
		biter = edma_hal_channel_reg_read(data->hal_cfg,
						  chan_id,
						  EDMA_TCD_BITER);
		done = edma_hal_channel_reg_read(data->hal_cfg,
						 chan_id,
						 EDMA_TCD_CH_CSR) & EDMA_TCD_CH_CSR_DONE_MASK;
		if (done) {
...
out_release_channel:
	 ...
	 */
	if (EDMA_HAS_MUX(data->hal_cfg)) {
		ret = edma_hal_set_channel_mux(data->hal_cfg, chan_id, 0);
		if (ret < 0) {
			LOG_ERR("failed to set channel MUX");
			return to_std_error(ret);
		}
...
static int edma_get_attribute(const struct device *dev, uint32_t type, uint32_t *val)
{
	...
	case DMA_ATTR_BUFFER_ADDRESS_ALIGNMENT:
		*val = CONFIG_DMA_NXP_IMX_EDMA_ALIGN;
		break;
